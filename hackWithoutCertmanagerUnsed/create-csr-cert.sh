...
O = zeng.dev
CN = validating-application-standards.default
...
kubectl get csr validating-application-standards.default -o jsonpath='{.spec.request}' | base64 --decode | \
  cfssl sign -ca tls.crt -ca-key tls.key -config $(dirname "${BASH_SOURCE[0]}")/server-signing-config.json - | \
  cfssljson -bare validating-application-standards-cert

# set status.certificate in certificate
kubectl get csr validating-application-standards.default -o json | \
  jq '.status.certificate = "'$(base64 -i validating-application-standards-cert.pem | tr -d '\n')'"' | \
  kubectl replace --raw /apis/certificates.k8s.io/v1/certificatesigningrequests/validating-application-standards.default/status -f -
